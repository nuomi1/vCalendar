...
describe("2.4 - inferInstrumentType", () => {
  test("688开头 -> 科", () =>
    expect(inferInstrumentType("688001", "stocks")).toBe("科"));
  test("300开头 -> 创", () =>
    expect(inferInstrumentType("300750", "stocks")).toBe("创"));
  test("6开头 -> 上", () =>
    expect(inferInstrumentType("600000", "stocks")).toBe("上"));
  test("000/001 -> 深", () =>
    expect(inferInstrumentType("001312", "stocks")).toBe("深"));
  test("8/4开头 -> 北", () =>
    expect(inferInstrumentType("830123", "stocks")).toBe("北"));
  test("bonds -> 债", () =>
    expect(inferInstrumentType("123456", "bonds")).toBe("债"));
  test("reits -> REITs", () =>
    expect(inferInstrumentType("508001", "reits")).toBe("REITs"));
});
...
describe("ical.js - ical.js 输出格式验证", () => {
  ...
  test("VEVENT 包含所有必需字段", () => {
    ...
    const ics = createICS([record], "stocks");
